...
func routes(_ app: Application) throws {
    app.post { req async throws -> String in
        ...
        print("[req] time=\(Date().millisecondsSince1970) id=\(request.id)")
        do {
            ...
            print("[res] time=\(Date().millisecondsSince1970) id=\(request.id)")
            ...
            let jsonEncoder = JSONEncoder()
            let jsonResultData = try? jsonEncoder.encode(resp)
            print("[res] time=\(Date().millisecondsSince1970) id=\(request.id) error=\(error)")
            return String(data: jsonResultData!, encoding: .utf8) ?? ""
        }    
    }
}
